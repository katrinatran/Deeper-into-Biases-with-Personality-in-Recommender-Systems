...
@author: making changes for project = Katrina
        ...
        - changing to fit data from paper "Personality Bias of Music Recommendation Algorithms"
        - changed to create preprocessing files for the data and fitting the problem that is addressed in this project
        - also added cross validation by creating the k-fold cross validation files from this preprocessing file
...
def main():
    ...
    for seed in tqdm(UN_SEEDS, desc='seeds'):
        ...
        dataset_tr_vd = os.path.join(pandas_dir_path, 'vd_tr_data.csv')
        dataset_te_vd = os.path.join(pandas_dir_path, 'vd_te_data.csv')
            ...
            with open(dataset, "r") as f:
                ...
                for data in reader:
                    ...
                    if userid in sess_clicks:
                        ...
                        sess_clicks[userid] = [item]
                    for j in range(1, int(data['play_count'])):
                        sess_clicks[userid] += [item]
                    ctr += 1
        ...
        tra_vd, tra_vd_seqs, item_dict_vd = make_tr_dataset(dataset_tr_vd)
        tes_vd = make_te_dataset(dataset_te_vd, item_dict_vd)
        ...
        pickle.dump(tra_vd, open(new_dir_name + '/validation_train.txt', 'wb'))
        pickle.dump(tes_vd, open(new_dir_name + '/validation_test.txt', 'wb'))
        print("DONE!")

        # creating a k-fold cross validation set of preprocessed data splits:
        full_test_dataset = os.path.join(pandas_dir_path, 'te_data.csv')
        with open(full_test_dataset, 'r') as r, open('shuffled.csv', 'w') as w:
            data = r.readlines()
        ...
        # split the files into 5 files
        csvfile = open('shuffled.csv', 'r').readlines()
        csv_reader = csv.reader(csvfile)
        header_of_file = next(csv_reader)
        joined_string = ",".join(header_of_file)
        joined_string = joined_string + '\n'
        filename = 1
        batchsize = int((len(csvfile) - 1) / 5)
        for i in range(1, len(csvfile)):
            if i == 1:
                if not os.path.exists('twitter/' + str(seed) + '/kfolds/'):
                    os.makedirs('twitter/' + str(seed) + '/kfolds/')
                with open('twitter/' + str(seed) + '/kfolds/filesplit' + str(filename) + '.csv', 'w+') as writeto:
                    writeto.writelines(joined_string)
                    writeto.writelines(csvfile[i:i + batchsize])
                filename += 1
            if i % batchsize == 0:
                if not os.path.exists('twitter/' + str(seed) + '/kfolds/'):
                    os.makedirs('twitter/' + str(seed) + '/kfolds/')
        ...
        # combine files to create training sets vs test
        heading = 'twitter/' + str(seed) + '/kfolds/'
        files_to_combine = [heading + 'filesplit1.csv', heading + 'filesplit2.csv', heading + 'filesplit3.csv',
                            heading + 'filesplit4.csv', heading + 'filesplit5.csv']
        for i in range(0, 5):
            files_combine_wo = files_to_combine.copy()
            files_combine_wo.pop(i)
            inputFileName = heading + "combined_output" + str(i+1) + ".csv"
            file0 = open(inputFileName, "a")
            file0.write(joined_string)
            for f in files_combine_wo:
                file1 = open(f, "r")
                next(file1)
                for line in file1:
                    file0.write(line)
        ...
        # processing to make the training and test files
        for i in range(0, 5):
            print("Creating k-fold validation files")
            training_data = heading + "combined_output" + str(i+1) + ".csv"
            testing_data = heading + "filesplit" + str(i+1) + ".csv"

            tra_fold, tra_seqs_fold, item_dict_fold = make_tr_dataset(training_data)
            print("training created")
            tes_fold = make_te_dataset(testing_data, item_dict_fold)
            print("testing created")
            if not os.path.exists(heading + 'final/'):
                os.makedirs(heading + 'final/')
            pickle.dump(tra_fold, open(heading + 'final/train' + str(i+1) + '.txt', 'wb'))
            pickle.dump(tes_fold, open(heading + 'final/test' + str(i+1) + '.txt', 'wb'))

    # understanding this code to be able to convert to get the data I needed:
